...
// plotting positions, and Latin hypercube sampling. Consumed by corehydropy.stats.
...
void register_stats(py::module_& m) {
    ...
    m.def("latin_hypercube", [](int sample_size, int dimension, int seed) {
        return corehydro::numerics::sampling::LatinHypercube::random(sample_size, dimension, seed);
    });
    m.def("latin_hypercube_median", [](int sample_size, int dimension, int seed) {
        return corehydro::numerics::sampling::LatinHypercube::median(sample_size, dimension, seed);
    });
}
