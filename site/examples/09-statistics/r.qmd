...
and base R already do well. The estimators are the same, so the base R versions
below reproduce the upstream summary table digit for digit:
...
`Statistics`, that class is internal to the port; base R's `cor()` computes the
same coefficients, and the seeded data is identical. Kendall's $\tau_b$ comes from
`cor(method = "kendall")` in R or `scipy.stats.kendalltau` in Python; on this data
it also equals 1.
...
- Hypothesis tests (one- and two-sample t, Mann-Whitney U, Kolmogorov-Smirnov):
  ...
  core, and base R's `t.test`, `wilcox.test`, and `ks.test` (or `scipy.stats` in
  Python) cover the same ground.
