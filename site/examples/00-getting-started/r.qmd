...
`bestfitr`, whose compiled core is a validated C++ port of the same library, so the
...
- How to install `bestfitr` and check the installation.
...
resolving the DLL path. None of that exists here. `bestfitr` ships the ported
...
delegate wrappers to pass functions into the C# samplers. With `bestfitr`, plain
