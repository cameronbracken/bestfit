...
class IUnivariateAnalysis : public IBayesianAnalysis, public IProbabilityOrdinates {
   public:
    ...
    virtual bestfit::numerics::distributions::UnivariateDistributionBase* get_distribution(
        int index) = 0;

    // The point-estimate distribution using the analysis's currently configured PointEstimator
    // (C# `GetPointEstimateDistribution()`).
    virtual bestfit::numerics::distributions::UnivariateDistributionBase*
    ...
    virtual bestfit::numerics::distributions::UnivariateDistributionBase*
    get_point_estimate_distribution(bestfit::estimation::PointEstimateType point_estimator) = 0;
};
