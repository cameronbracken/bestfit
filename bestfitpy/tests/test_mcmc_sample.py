...
Cross-language identity with bestfitr is proven by the mcmc_sampler fixtures; here we
...
def test_result_is_complete_and_consistent():
    x = bf.Distribution("Normal", [100, 15]).random(100, seed=42)
    fit = bf.mcmc_sample(x, "Normal", sampler="DEMCzs", iterations=1000, seed=12345)
...
def test_seed_determinism():
    x = bf.Distribution("Gumbel", [100, 10]).random(80, seed=9)
    a = bf.mcmc_sample(x, "Gumbel", sampler="RWMH", iterations=500, seed=12345)
    b = bf.mcmc_sample(x, "Gumbel", sampler="RWMH", iterations=500, seed=12345)
    ...
    c = bf.mcmc_sample(x, "Gumbel", sampler="RWMH", iterations=500, seed=999)
    assert not np.array_equal(a["posterior_mean"], c["posterior_mean"])


def test_argument_validation():
    x = bf.Distribution("Normal", [0, 1]).random(30, seed=1)
    with pytest.raises(ValueError, match="unknown sampler"):
        bf.mcmc_sample(x, "Normal", sampler="Gibbs")
    with pytest.raises(Exception):
        bf.mcmc_sample(x, "Normal", initialize="Nope")
