...
```jsonc
{
  "target":  "Dirichlet",                   // "Dirichlet" | "Multinomial" | "BivariateEmpirical"
                                             // | "MultivariateNormal" (later: "MultivariateStudentT")
...
- `MultivariateNormal`: `{"mean": [..], "covariance": [[..]]}` plus an optional `"seed": <int>` --
  see Statefulness below. (`MultivariateStudentT` is design-only, not yet implemented -- it will
  add its own task later.)
...
provenance for that one value -- used by MultivariateNormal's curated (not-lifted-verbatim-from-a-
C#-test-literal) cases, whose expected values were generated by running the ported logic itself
against `tools/oracle_emitter --dump` and confirming them against the real Numerics library rather
than copied from a C# test file; see `"source": "curated via oracle_emitter --dump"` throughout
`fixtures/distributions/multivariate/multivariate_normal.json`. Methods:
...
- `mahalanobis [x...]` (MultivariateNormal only) -- Mahalanobis distance of the point `x` from the
  distribution's mean.
- `inverse_cdf [[p...], i]` (MultivariateNormal only) -- inverse CDF at the probability vector `p`,
  asserted element-wise via the trailing index arg (0-based); stateless (fresh instance per call).
...
per-assertion dispatch, unchanged. Seeded *sampling* methods (`random_value`/`lhs_value`-style
first-N draws) are not covered yet -- they arrive in a later task (Chunk 10).
