...
// Length mismatch and fewer-than-two ordinates are both invalid (ValidateData's first two
// checks).
void test_length_mismatch_and_too_few_points_invalid() {
    EmpiricalDistribution mismatched({100.0, 125.0, 150.0}, {0.1, 0.9});
    CHECK_TRUE(!mismatched.parameters_valid());
...
int main() {
    ...
    test_length_mismatch_and_too_few_points_invalid();
