// Transcribed C# oracle tests for the local optimizers (Task B5; B6 extends this file with
// Powell, and MLSL lands in its own global-optimizer section):
//   upstream/Numerics/Test_Numerics/Mathematics/Optimization/Local/Test_BFGS.cs @ a2c4dbf
//
// All 8 upstream [TestMethod]s are transcribed with their exact fitness + coordinate
// oracles and tolerances (1E-4 throughout, except the SumOfPowerFunctions coordinates at
// 1E-3), unaltered. These are internal-support ports validated against the C# test oracles
// themselves, so there is no fixtures/ entry for this file (fixtures/ is the public
// estimation API surface only). Skipped upstream methods: none (Test_BFGS.cs has no XML or
