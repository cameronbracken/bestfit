...
namespace bestfit::analyses::bivariate {

class CoincidentFrequencyAnalysis : public AnalysisBase {
   public:
    using BivariateAnalysis = bestfit::analyses::BivariateAnalysis;
    ...
    using Normal = bestfit::numerics::distributions::Normal;
    ...
    void create_frequency_analysis_results() {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            r.mean_curve[static_cast<std::size_t>(k)] = bestfit::numerics::data::mean(aeps);
            std::sort(aeps.begin(), aeps.end());
            r.confidence_intervals[static_cast<std::size_t>(k)][0] =
                bestfit::numerics::data::percentile(aeps, alpha / 2.0, true);
            r.confidence_intervals[static_cast<std::size_t>(k)][1] =
                bestfit::numerics::data::percentile(aeps, 1.0 - alpha / 2.0, true);
        }
