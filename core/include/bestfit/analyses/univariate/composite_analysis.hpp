...
//   4. Validate() returns bestfit::models::ValidationResult (the (bool, List<string>) tuple),
...
class CompositeAnalysis : public AnalysisBase, public IUnivariateAnalysis {
   public:
    ...
    using CompetingRisks = bestfit::numerics::distributions::CompetingRisks;
    using Mixture = bestfit::numerics::distributions::Mixture;
    ...
    using DependencyType = bestfit::numerics::data::probability::DependencyType;
    using PointEstimateType = bestfit::estimation::PointEstimateType;
    using Transform = bestfit::numerics::data::Transform;
    ...
    CompositeAnalysis()
        : placeholder_model_(std::make_unique<bestfit::models::UnivariateDistributionModel>()),
    ...
    void run() override {
        bestfit::models::ValidationResult validation = validate();
    ...
    bestfit::models::ValidationResult validate() const override {
        ...
        if (analyses_.empty()) {
            ...
            if (composite_distribution_type_ == CompositeType::Mixture &&
                sum - 1.0 > bestfit::numerics::kDoubleMachineEpsilon * 2.0) {
                result.is_valid = false;
                result.validation_messages.push_back(
   ...
   private:
    ...
    std::unique_ptr<bestfit::models::UnivariateDistributionModel> placeholder_model_;
