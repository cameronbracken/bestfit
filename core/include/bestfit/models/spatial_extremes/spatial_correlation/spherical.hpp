// ported from: RMC-BestFit/src/RMC.BestFit/Models/SpatialExtremes/SpatialCorrelation/Spherical.cs @ fc28c0c
//
// Spherical spatial correlation function with compact support:
//   rho(h) = 1 - 1.5(h/phi) + 0.5(h/phi)^3  for h <= phi,  0 otherwise.
// One parameter, Range. See i_correlation_model.hpp for the base-vs-impl DRY decision and the
// ToXElement skip.
#pragma once
#include <cmath>
#include <memory>
#include <stdexcept>
#include <vector>

#include "bestfit/models/spatial_extremes/spatial_correlation/correlation_function_type.hpp"
#include "bestfit/models/spatial_extremes/spatial_correlation/i_correlation_model.hpp"
#include "bestfit/models/support/model_parameter.hpp"
#include "bestfit/numerics/distributions/uniform.hpp"
#include "bestfit/numerics/tools.hpp"

namespace bestfit::models::spatial_extremes {

class Spherical : public ICorrelationModel {
   public:
    // Constructs a new spherical correlation model (C# ctor):
    // Range (Value 10.0, LowerBound machine-eps, UpperBound 500, IsPositive, Uniform(eps, 500)).
    Spherical() {
        parameters_.emplace_back(
            /*owner_name=*/"", /*name=*/"Range", /*value=*/10.0,
            /*lower_bound=*/numerics::kDoubleMachineEpsilon, /*upper_bound=*/500.0,
            std::make_unique<numerics::distributions::Uniform>(numerics::kDoubleMachineEpsilon, 500.0),
            /*is_positive=*/true);
    }

    CorrelationFunctionType type() const override { return CorrelationFunctionType::Spherical; }

    // Evaluates the correlation function at distance h (C# Evaluate):
    // h<0 throws; h==0 -> 1; range<=0 -> 0; ratio>=1 -> 0; else 1 - 1.5*ratio + 0.5*ratio^3.
    double evaluate(double h) const override {
        if (h < 0) throw std::invalid_argument("Distance must be non-negative.");
        if (h == 0) return 1.0;
        double range = parameters_[0].value();
        if (range <= 0) return 0.0;
        double ratio = h / range;
        if (ratio >= 1.0) return 0.0;
        return 1.0 - 1.5 * ratio + 0.5 * std::pow(ratio, 3);
    }

    // Returns a deep copy (C# Clone): copies Value/LowerBound/UpperBound and clones the prior.
    std::unique_ptr<ICorrelationModel> clone() const override {
        auto result = std::make_unique<Spherical>();
        result->parameters_[0].set_value(parameters_[0].value());
        result->parameters_[0].set_lower_bound(parameters_[0].lower_bound());
        result->parameters_[0].set_upper_bound(parameters_[0].upper_bound());
        result->parameters_[0].set_prior_distribution(parameters_[0].prior_distribution().clone());
        return result;
    }
};

}  // namespace bestfit::models::spatial_extremes
