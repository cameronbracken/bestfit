...
// Standard normal inverse CDF (Wichura AS241). Used by Beta::IncompleteInverse.
// r8poly_value evaluates p(x) = a[0] + a[1]*x + ... + a[n-1]*x^(n-1).
inline double r8poly_value_sf(int n, const double a[], double x) {
    double value = 0.0;
...
inline double normal_standard_z(double p) {
    ...
    if (std::fabs(q) <= 0.425) {
        r = 0.180625 - q * q;
        value = q * r8poly_value_sf(8, a, r) / r8poly_value_sf(8, b, r);
    } else {
        r = q < 0.0 ? p : 1.0 - p;
        r = std::sqrt(-std::log(r));
        if (r <= 5.0) {
            r = r - 1.6;
            value = r8poly_value_sf(8, c, r) / r8poly_value_sf(8, d, r);
        } else {
            r = r - 5.0;
            value = r8poly_value_sf(8, e, r) / r8poly_value_sf(8, f, r);
        }
...
// Natural log of the Gamma function, using Lanczos approximation for accuracy.
// Delegates to std::lgamma which matches the C# Math.Log(Gamma.Function(x)) pattern.
