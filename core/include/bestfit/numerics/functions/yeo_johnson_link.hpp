...
// ported YeoJohnson transform class (bestfit/numerics/data/yeo_johnson.hpp); lambda is
...
class YeoJohnsonLink : public ILinkFunction {
   public:
    ...
    explicit YeoJohnsonLink(const std::vector<double>& values) {
        ...
        for (std::size_t i = 0; i < values.size(); ++i)
            if (!bestfit::numerics::is_finite(values[i]))
   ...
   private:
    ...
    static double validate_lambda(double lambda) {
        if (!bestfit::numerics::is_finite(lambda) || lambda < kMinimumLambda ||
