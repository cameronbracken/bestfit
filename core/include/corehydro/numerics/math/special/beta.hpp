...
namespace gamma_sf = corehydro::numerics::math::special;
...
inline double incbcf(double a, double b, double x) {
    ...
    ans = 1.0;
    r = 1.0;
    thresh = 3.0 * corehydro::numerics::kDoubleMachineEpsilon;
    for (int i = 1; i <= 300; ++i) {
        xk = -(x * k1 * k2) / (k3 * k4);
        pk = pkm1 + pkm2 * xk;
        qk = qkm1 + qkm2 * xk;
...
inline double power_series(double a, double b, double x) {
    ...
    n  = 2.0;
    s  = 0.0;
    z  = corehydro::numerics::kDoubleMachineEpsilon * ai;
    while (std::fabs(v) > z) {
        u  = (n - b) * x / n;
        t *= u;
        v  = t / (a + n);
...
inline double incomplete(double a, double b, double x) {
    ...
    if (flag && bb * xx <= 1.0 && xx <= 0.95) {
        ...
        t = (t <= corehydro::numerics::kDoubleMachineEpsilon)
...
inline double incomplete_inverse(double aa, double bb, double yy0) {
    ...
    if (aa <= 1.0 || bb <= 1.0) {
        nflg = true;
        dithresh = 4.0 * corehydro::numerics::kDoubleMachineEpsilon;
        rflg = false;
        a = aa; b = bb; y0 = yy0;
        x = a / (a + b);
        y = incomplete(a, b, x);
    ...
    yp = -corehydro::numerics::math::special::detail::normal_standard_z(yy0);
    if (yy0 > 0.5) {
        rflg = true;
...
ihalve:
    ...
    for (i = 0; i < 400; ++i) {
        if (i != 0) {
            x  = x0 + di * (x1 - x0);
            if (x == 1.0) x = 1.0 - corehydro::numerics::kDoubleMachineEpsilon;
            y  = incomplete(a, b, x);
            yp = (x1 - x0) / (x1 + x0);
        ...
        if (y < y0) {
            ...
            x1 = x;
            if (rflg && x1 < corehydro::numerics::kDoubleMachineEpsilon) {
                x0 = 0.0; goto done;
            }
            yh = y;
            if (dir > 0) { dir = 0; di = 0.5; }
    ...
    if (x0 >= 1.0) { x0 = 1.0 - corehydro::numerics::kDoubleMachineEpsilon; goto done; }
...
newt:
    ...
    for (i = 0; i < 10; ++i) {
        ...
        if (x0 >= 1.0) { x0 = 1.0 - corehydro::numerics::kDoubleMachineEpsilon; goto done; }
        if (std::fabs(d / x0) < 64.0 * corehydro::numerics::kDoubleMachineEpsilon) goto done;
    }
