...
namespace corehydro::numerics::math::differentiation {
...
inline double calculate_step_size(double x, int order = 1) {
    ...
    return std::pow(corehydro::numerics::kDoubleMachineEpsilon, 1.0 / (1.0 + order)) *
           (1.0 + std::fabs(x));
}
...
inline std::vector<double> gradient(const ScalarFunction& f, const std::vector<double>& theta,
    ...
    double f0 = f(theta0);
    if (!corehydro::numerics::is_finite(f0)) throw std::domain_error("f(theta) is not finite.");
        ...
        for (int backtrack = 0; backtrack <= max_backtrack && !success; ++backtrack) {
            ...
            try {
                if (can_central) {
                    ...
                    if (!corehydro::numerics::is_finite(f_plus) || !corehydro::numerics::is_finite(f_minus)) {
                        hj *= 0.5;
                        continue;
                    }
                    ...
                    double f_plus = f(theta_plus);
                    if (!corehydro::numerics::is_finite(f_plus)) {
                        hj *= 0.5;
                        continue;
                    }
                    ...
                    double f_minus = f(theta_minus);
                    if (!corehydro::numerics::is_finite(f_minus)) {
                        hj *= 0.5;
                        continue;
                    }
        ...
        for (int backtrack = 0; backtrack <= max_backtrack && !success; ++backtrack) {
            ...
            try {
                if (can_central) {
                    ...
                    double f1 = f(x1);
                    double f2 = f(x2);
                    if (!corehydro::numerics::is_finite(f1) || !corehydro::numerics::is_finite(f2)) {
                        hj *= 0.5;
                        continue;
                    }
            ...
            for (int backtrack = 0; backtrack <= max_backtrack && !success; ++backtrack) {
                ...
                try {
                    if (central_i && central_j) {
                        ...
                        double f_mp = f(x_mp);
                        double f_mm = f(x_mm);
                        if (!corehydro::numerics::is_finite(f_pp) || !corehydro::numerics::is_finite(f_pm) ||
                            !corehydro::numerics::is_finite(f_mp) || !corehydro::numerics::is_finite(f_mm)) {
                            hi *= 0.5;
                            hj *= 0.5;
                            continue;
                        }
                        ...
                        if (forward_forward) {
                            ...
                            double f_j = f(x_j);
                            double f_ij = f(x_ij);
                            if (!corehydro::numerics::is_finite(f_i) || !corehydro::numerics::is_finite(f_j) ||
                                !corehydro::numerics::is_finite(f_ij)) {
                                hi *= 0.5;
                                hj *= 0.5;
                                continue;
                            }
