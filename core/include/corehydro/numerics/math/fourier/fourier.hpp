...
namespace corehydro::numerics::math::fourier {
...
inline void fft(std::vector<double>& data, bool inverse = false) {
    ...
    if (n < 2 || !corehydro::numerics::is_power_of_two(n))
    ...
    while (nn > mmax) {
        int istep = mmax << 1;
        double theta = isign * (2.0 * corehydro::numerics::kPi / mmax);
...
inline void real_fft(std::vector<double>& data, bool inverse = false) {
    ...
    int isign = inverse ? -1 : 1;
    double theta = corehydro::numerics::kPi / (n >> 1);
    double c1 = 0.5, c2;
    if (isign == 1) {
        c2 = -0.5;
        fft(data);
...
inline std::optional<std::vector<std::array<double, 2>>> autocorrelation(const std::vector<double>& series,
    ...
    double m = corehydro::numerics::data::mean(series);
