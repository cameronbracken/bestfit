...
// static_assert to derive from both `corehydro::models::ModelBase` and
// `corehydro::models::ISimulatable<std::vector<double>>` (the compile-time analogue of the C#
...
inline double skewness(const std::vector<double>& data) {
    ...
    double mean = corehydro::numerics::data::mean(data);
...
class PosteriorPredictiveCheck {
    static_assert(std::is_base_of<corehydro::models::ModelBase, TModel>::value,
                  "TModel must derive from corehydro::models::ModelBase.");
    static_assert(
        std::is_base_of<corehydro::models::ISimulatable<std::vector<double>>, TModel>::value,
   ...
   public:
    ...
    std::vector<std::vector<double>> generate_replicates(int number_of_replicates) const {
        ...
        auto indices = corehydro::numerics::utilities::next_integers(
            rng, 0, static_cast<int>(posterior_samples_.size()), number_of_replicates);
        auto seeds = corehydro::numerics::utilities::next_integers(rng, number_of_replicates);
    ...
    PredictiveCheckResults compute_common_p_values(int number_of_replicates = 1000) const {
        ...
        double obs_mean = corehydro::numerics::data::mean(observed_data_);
        double obs_sd = corehydro::numerics::data::standard_deviation(observed_data_);
        ...
        for (const auto& y_rep : replicates) {
            if (corehydro::numerics::data::mean(y_rep) >= obs_mean) ++count_mean;
            if (corehydro::numerics::data::standard_deviation(y_rep) >= obs_sd) ++count_sd;
   ...
   private:
    ...
    static PredictiveSummary compute_summary_from_data(
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            means[i] = corehydro::numerics::data::mean(valid);
            sds[i] = corehydro::numerics::data::standard_deviation(valid);
    ...
    static std::vector<double> percentiles(std::vector<double> data,
        ...
        for (std::size_t i = 0; i < ks.size(); ++i)
            result[i] = corehydro::numerics::data::percentile(data, ks[i], /*data_is_sorted=*/true);
        return result;
    }
