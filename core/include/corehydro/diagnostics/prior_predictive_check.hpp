...
// by static_assert to derive from both `corehydro::models::ModelBase` (for `parameters()` /
// `prior_log_likelihood()`) and `corehydro::models::ISimulatable<std::vector<double>>` (for
...
class PriorPredictiveCheck {
   ...
   public:
    ...
    std::vector<ParameterSet> sample_from_priors() const {
        ...
        auto uniforms = corehydro::numerics::utilities::next_doubles(rng, number_of_draws_, num_params);
    ...
    std::vector<std::vector<double>> generate_prior_predictive(int sample_size) const {
        ...
        auto seeds = corehydro::numerics::utilities::next_integers(rng, num_samples);
