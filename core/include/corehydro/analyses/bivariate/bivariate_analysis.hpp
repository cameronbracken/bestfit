...
class BivariateAnalysis : public AnalysisBase, public IBayesianAnalysis {
   public:
    using BivariateDistribution = corehydro::models::BivariateDistribution;
    ...
    void create_frequency_analysis_results() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            r.mean_curve[ui] = corehydro::numerics::data::mean(p);
            std::sort(p.begin(), p.end());
            r.confidence_intervals[ui][0] =
                corehydro::numerics::data::percentile(p, alpha / 2.0, true);
            r.confidence_intervals[ui][1] =
                corehydro::numerics::data::percentile(p, 1.0 - alpha / 2.0, true);
        }
    ...
    void update_point_estimate_results() {
        ...
        const corehydro::models::ExactSeries& series_x = marginal_x->data_frame().exact_series();
        const corehydro::models::ExactSeries& series_y = marginal_y->data_frame().exact_series();
        std::vector<const corehydro::models::ExactData*> data_x = non_low_outlier(series_x);
        std::vector<const corehydro::models::ExactData*> data_y = non_low_outlier(series_y);
    ...
    corehydro::models::ValidationResult validate() const override {
        ...
        corehydro::models::ValidationResult dist_valid = bivariate_distribution_->validate();
   ...
   private:
    ...
    static std::vector<const corehydro::models::ExactData*> non_low_outlier(
        const corehydro::models::ExactSeries& series) {
        std::vector<const corehydro::models::ExactData*> result;
