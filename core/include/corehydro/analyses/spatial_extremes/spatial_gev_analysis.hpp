...
#include "corehydro/analyses/spatial_extremes/spatial_gev_cross_validation_results.hpp"
#include "corehydro/analyses/spatial_extremes/spatial_gev_site_results.hpp"
...
class SpatialGEVAnalysis : public AnalysisBase, public IBayesianAnalysis {
   public:
    using SpatialGEV = corehydro::models::spatial_extremes::SpatialGEV;
    ...
    using GeneralizedExtremeValue = corehydro::numerics::distributions::GeneralizedExtremeValue;
    ...
    SpatialGEVSiteResults predict_at_ungauged_location(
        ...
        for (int p = 0; p < n_probs; ++p) {
            ...
            result.quantile_mode[static_cast<std::size_t>(p)] =
                corehydro::numerics::data::percentile(q_vals, 0.5);
        }
    ...
    void run_cross_validation() {
        ...
        try {
            for (int j = 0; j < sites; ++j) {
                ...
                if (!site_data.empty()) {
                    ...
                    gev.estimate(site_data,
                                 corehydro::numerics::distributions::EstimationMethod::MaximumLikelihood);
            ...
            cv.mean_absolute_error = corehydro::numerics::data::mean(abs_err);
            cv.root_mean_square_error = std::sqrt(corehydro::numerics::data::mean(sq_err));
            cv.mean_bias = corehydro::numerics::data::mean(cv.site_bias);
    ...
    void run_spatial_bootstrap(int n_bootstrap = 200, int block_size = 0) {
        ...
        for (int b = 0; b < n_bootstrap; ++b) {
            ...
            corehydro::estimation::BayesianAnalysis boot_bayes(boot_model);
        ...
        for (int j = 0; j < n_sites; ++j) {
            ...
            if (valid_loc.size() >= 10) {
                ...
                sr.location_lower = corehydro::numerics::data::percentile(valid_loc, alpha / 2, true);
                sr.location_upper =
                    corehydro::numerics::data::percentile(valid_loc, 1 - alpha / 2, true);
                sr.scale_lower = corehydro::numerics::data::percentile(valid_scl, alpha / 2, true);
                sr.scale_upper = corehydro::numerics::data::percentile(valid_scl, 1 - alpha / 2, true);
                sr.shape_lower = corehydro::numerics::data::percentile(valid_shp, alpha / 2, true);
                sr.shape_upper = corehydro::numerics::data::percentile(valid_shp, 1 - alpha / 2, true);
                ...
                for (int p = 0; p < n_probs; ++p) {
                    ...
                    if (valid_q.size() >= 10) {
                        ...
                        sr.quantile_lower[static_cast<std::size_t>(p)] =
                            corehydro::numerics::data::percentile(valid_q, alpha / 2, true);
                        sr.quantile_upper[static_cast<std::size_t>(p)] =
                            corehydro::numerics::data::percentile(valid_q, 1 - alpha / 2, true);
                    }
                }
            }
        }
    ...
    corehydro::models::ValidationResult validate() const override {
        ...
        corehydro::models::ValidationResult model_valid = spatial_gev_->validate();
   ...
   private:
    ...
    static Band band(std::vector<double> vals, double alpha) {
        ...
        return {corehydro::numerics::data::mean(vals),
                corehydro::numerics::data::percentile(vals, alpha / 2.0, true),
                corehydro::numerics::data::percentile(vals, 1.0 - alpha / 2.0, true)};
    }
    ...
    std::vector<int> create_spatial_blocks(int block_size) const {
        ...
        corehydro::numerics::sampling::MersenneTwister prng(12345);
        ...
        for (int block = 0; block < n_blocks && !unassigned.empty(); ++block) {
            ...
            while (site_count < block_size && !unassigned.empty()) {
                ...
                for (std::size_t idx = 0; idx < unassigned.size(); ++idx) {
                    ...
                    double dist = corehydro::numerics::distance(
