...
//      `point_estimate()` is a corehydro ADDITION centralizing the `PointEstimator ==
...
//      `corehydro::numerics::data::RunningCovarianceMatrix`. `estimate()` now calls the three
...
class BayesianAnalysis {
   public:
    ...
    explicit BayesianAnalysis(corehydro::models::ModelBase& model, SamplerType type = SamplerType::DEMCzs)
    ...
    corehydro::models::ModelBase& model() { return model_; }
    const corehydro::models::ModelBase& model() const { return model_; }
    ...
    // Returns the posterior point estimate selected by `point_estimator()` (corehydro
    ...
    void set_default_simulation_options() {
        ...
        iterations_ = corehydro::numerics::sampling::mcmc::minimum_sample_size(ci, 0.01, 1.0 - alpha);
    ...
    void set_up_sampler() {
        ...
        std::vector<std::shared_ptr<corehydro::numerics::distributions::UnivariateDistributionBase>> priors;
        priors.reserve(static_cast<std::size_t>(model_.number_of_parameters()));
        for (auto& parameter : model_.parameters()) {
            priors.push_back(std::shared_ptr<corehydro::numerics::distributions::UnivariateDistributionBase>(
        ...
        corehydro::numerics::sampling::mcmc::LogLikelihood posterior = [this](const std::vector<double>& p) {
        ...
        switch (type_) {
            case SamplerType::DEMCz: {
                auto sampler =
                    std::make_unique<corehydro::numerics::sampling::mcmc::DEMCz>(priors, posterior);
                sampler->jump = jump_;
            ...
            case SamplerType::DEMCzs: {
                auto sampler =
                    std::make_unique<corehydro::numerics::sampling::mcmc::DEMCzs>(priors, posterior);
                sampler->jump = jump_;
                sampler->jump_threshold = jump_threshold_;
            ...
            case SamplerType::ARWMH: {
                auto sampler =
                    std::make_unique<corehydro::numerics::sampling::mcmc::ARWMH>(priors, posterior);
            ...
            case SamplerType::NUTS: {
                ...
                sampler_ = std::make_unique<corehydro::numerics::sampling::mcmc::NUTS>(
    ...
    // `corehydro::numerics::distributions::GeneralizedPareto::mle()`, which returns
    ...
    // `corehydro::numerics::data::RunningCovarianceMatrix` (Welford's online algorithm) and
    ...
    std::optional<corehydro::numerics::math::linalg::Matrix> get_posterior_covariance_matrix() const {
        if (!is_estimated_ || !results_ || results_->output.size() < 2) return std::nullopt;

        corehydro::numerics::data::RunningCovarianceMatrix rcm(model_.number_of_parameters());
    ...
    std::optional<corehydro::numerics::math::linalg::Matrix> get_posterior_correlation_matrix() const {
        if (!is_estimated_ || !results_ || results_->output.size() < 2) return std::nullopt;
    ...
    corehydro::diagnostics::InfluenceDiagnostics compute_influence_diagnostics() const {
        if (!is_estimated_ || !results_)
            throw std::invalid_argument(
                "Estimation must be completed before computing influence diagnostics.");

        if (pareto_k_.empty()) return corehydro::diagnostics::InfluenceDiagnostics();
        ...
        return corehydro::diagnostics::InfluenceDiagnostics(pareto_k_, elpd_loo_,
    ...
    corehydro::diagnostics::PriorInfluenceDiagnostics compute_prior_influence_diagnostics(
        int thin_every = 10) const {
        if (!is_estimated_ || !results_)
            throw std::invalid_argument(
                "Estimation must be completed before computing prior influence diagnostics.");
        return corehydro::diagnostics::PriorInfluenceDiagnostics(model_, *results_, thin_every);
    }
    ...
    corehydro::diagnostics::LeverageDiagnostics compute_leverage_diagnostics() const {
        ...
        return corehydro::diagnostics::LeverageDiagnostics(model_, results_->map.values);
    }
   ...
   private:
    ...
    static double pareto_smooth_weights(std::vector<double>& log_weights, int m) {
        ...
        // Fit the GPD by MLE (Numerics/corehydro's Hosking parameterization: Kappa has the
        ...
        double sigma;
        try {
            corehydro::numerics::distributions::GeneralizedPareto gpd_mle;
