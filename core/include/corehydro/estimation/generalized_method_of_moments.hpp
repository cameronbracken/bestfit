...
class GeneralizedMethodOfMoments {
   public:
    ...
    using Vector = corehydro::numerics::math::linalg::Vector;
    ...
    explicit GeneralizedMethodOfMoments(corehydro::models::IGMMModel& model,
    ...
    corehydro::models::IGMMModel* model() const { return model_; }
    ...
    double q(const std::vector<double>& parameters) const {
        ...
        return corehydro::numerics::is_finite(qv) ? qv : std::numeric_limits<double>::max();
    }
    ...
    Matrix get_s(const std::vector<double>& parameters) const {
        ...
        s = corehydro::numerics::math::linalg::MatrixRegularization::
    ...
    Matrix get_covariance(const std::vector<double>& parameters, bool sandwich = true) {
        try {
            ...
            s_ = get_s(parameters);
            s_ = corehydro::numerics::math::linalg::MatrixRegularization::regularize(*s_);
            w_ = s_->inverse();
            ...
            bread = corehydro::numerics::math::linalg::MatrixRegularization::
            ...
            if (!sandwich) {
                ...
                bread_inv = corehydro::numerics::math::linalg::MatrixRegularization::
            ...
            covariance = corehydro::numerics::math::linalg::MatrixRegularization::
    ...
    std::vector<Matrix> profile_q(int bins = 100, bool true_profile = true) const {
        ...
        for (int i = 0; i < number_of_parameters_; ++i) {
            ...
            auto seq = corehydro::numerics::sampling::Stratify::XValues(
                corehydro::numerics::sampling::StratificationOptions(lower_bounds_[si],
    ...
    corehydro::diagnostics::InfluenceDiagnostics get_influence_diagnostics() {
        std::vector<double> cooks_d = get_cooks_distance();
        std::vector<corehydro::diagnostics::InfluenceDiagnostics::ObservationInfluence> observations;
        ...
        for (std::size_t i = 0; i < cooks_d.size(); ++i)
            observations.emplace_back(static_cast<int>(i), cooks_d[i],
                                      std::numeric_limits<double>::quiet_NaN());
        return corehydro::diagnostics::InfluenceDiagnostics(std::move(observations));
    }

    // Influence diagnostics (Cook's distance in ParetoK) with data component metadata for labeling
    // (C# 1770).
    corehydro::diagnostics::InfluenceDiagnostics get_influence_diagnostics(
        const std::vector<corehydro::models::DataComponent>& data_components) {
        std::vector<double> cooks_d = get_cooks_distance();
        ...
        for (int i = 0; i < n; ++i) {
            const corehydro::models::DataComponent& dc = data_components[static_cast<std::size_t>(i)];
    ...
    corehydro::diagnostics::LeverageDiagnostics get_leverage_diagnostics() const {
        using LD = corehydro::diagnostics::LeverageDiagnostics;
        namespace linalg = corehydro::numerics::math::linalg;
        ...
        for (int i = 0; i < n_components; ++i) {
            ...
            observations.emplace_back(i, leverage, 0.0, agg_fit_influence[si],
                                      ...
                                      corehydro::models::DataComponentType::Exact, 1, std::nullopt);
        }
    ...
    void post_process(bool use_sandwich = true, bool compute_jstat = false) {
        ...
        corehydro::numerics::distributions::ChiSquared chi_squared(degree_of_freedom());
   ...
   private:
    ...
    void estimate_iterative() {
        ...
        for (gmm_iterations_ = 2; gmm_iterations_ <= max_gmm_iterations_; ++gmm_iterations_) {
            ...
            double distance = corehydro::numerics::distance(new_values, old_values);
    ...
    corehydro::models::IGMMModel* model_ = nullptr;  // C# `IGMMModel? Model` (241; non-owning, B9)
