// ported from: RMC-BestFit/src/RMC.BestFit/Models/TrendFunctions/CubicTrend.cs @ fc28c0c
//
// Cubic trend model:
//   y(t) = alpha + beta * (t - StartIndex) + gamma * (t - StartIndex)^2
//          + delta * (t - StartIndex)^3.
...
class CubicTrend final : public TrendModelBase {
   public:
    CubicTrend() { set_default_parameters(); }

    TrendModelType type() const override { return TrendModelType::Cubic; }
    ...
    void set_default_parameters() override {
        ...
        {
            ModelParameter p;  // "(delta)": bounds [-1, 1], Uniform(-1, 1) prior
            p.set_owner_name(owner_name());
            p.set_name("(\xCE\xB4)");
            p.set_lower_bound(-1.0);
            p.set_upper_bound(1.0);
    ...
    std::unique_ptr<ITrendModel> clone() const override {
        auto model = std::make_unique<CubicTrend>();
