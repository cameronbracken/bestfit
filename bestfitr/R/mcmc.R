# Public wrapper over the MCMC glue in src/mcmc.cpp (bf_mcmc_run_).
...
#' run reproduces the C# sampler stream bit-for-bit (and matches `bestfitpy`
...
mcmc_sample <- function(
  ...
  raw <- bf_mcmc_run_(sampler, "uniform_constraints", distribution, as.double(data), settings)

  list(
    parameters = unlist(bf_dist_parameter_names_(distribution)$short),
    chains = raw$chains,
