...
box_cox_lambda <- function(x) {
  bf_box_cox_lambda_(as.double(x))
}

#' @rdname box_cox
#' @export
box_cox <- function(x, lambda) {
  bf_box_cox_(as.double(x), lambda)
}

#' @rdname box_cox
#' @export
box_cox_inverse <- function(x, lambda) {
  bf_box_cox_inverse_(as.double(x), lambda)
}
...
yeo_johnson_lambda <- function(x) {
  bf_yeo_johnson_lambda_(as.double(x))
}

#' @rdname yeo_johnson
#' @export
yeo_johnson <- function(x, lambda) {
  bf_yeo_johnson_(as.double(x), lambda)
}

#' @rdname yeo_johnson
#' @export
yeo_johnson_inverse <- function(x, lambda) {
  bf_yeo_johnson_inverse_(as.double(x), lambda)
}
...
plotting_positions <- function(
  ...
  if (!is.null(alpha)) {
    return(bf_plotting_positions_alpha_(as.integer(n), as.double(alpha)))
  }
  method <- match.arg(method)
  bf_plotting_positions_(as.integer(n), method)
}
...
latin_hypercube <- function(n, dimension = 1, seed = NULL, median = FALSE) {
  ...
  flat <- bf_latin_hypercube_(as.integer(n), as.integer(dimension), seed, isTRUE(median))
