...
// port keeps that flat namespace (`bestfit::numerics::sampling`) for parity.
//
...
#include "bestfit/numerics/sampling/bootstrap/bootstrap_results.hpp"
...
namespace opt = bestfit::numerics::math::optimization;
...
class Bootstrap {
            ...
            for (int retry = 0; retry < max_retries; ++retry) {
                try {
                    ...
                    for (int j = 0; j < num_stats_; ++j) {
                        ...
                        for (double v : col)
                            if (bestfit::numerics::is_finite(v)) valid_col.push_back(v);
   ...
   private:
    ...
    BootstrapStatisticResult compute_percentile_ci(const std::vector<double>& values, double population_estimate,
        ...
        for (double v : values)
            if (bestfit::numerics::is_finite(v)) valid.push_back(v);
    ...
    BootstrapStatisticResult compute_normal_ci(const std::vector<double>& values, double population_estimate,
        ...
        for (double v : values)
            if (bestfit::numerics::is_finite(v)) valid_slice.push_back(apply_transform(v));
    ...
    BootstrapStatisticResult compute_bootstrap_t_ci(int statistic_index, double population_estimate,
        ...
        for (double v : x_col)
            if (bestfit::numerics::is_finite(v)) valid_x.push_back(v);
        for (double v : t_col)
            if (bestfit::numerics::is_finite(v)) valid_t.push_back(v);
    ...
    static bool contains_only_finite_values(const std::vector<double>& values) {
        for (double v : values)
            if (!bestfit::numerics::is_finite(v)) return false;
        return true;
    }
