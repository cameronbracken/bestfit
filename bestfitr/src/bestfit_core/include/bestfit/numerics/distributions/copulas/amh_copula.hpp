...
// (bestfit::numerics::math::rootfinding::solve). set_theta_from_tau is also a Brent
...
class AMHCopula : public ArchimedeanCopula {
   public:
    ...
    void set_theta_from_tau(const std::vector<double>& sample_data_x,
        ...
        double L = tau > 0.0 ? 0.001 : -1.0 + bestfit::numerics::kDoubleMachineEpsilon;
        double U = tau > 0.0 ? 1.0 - bestfit::numerics::kDoubleMachineEpsilon : -0.001;

        set_theta(bestfit::numerics::math::rootfinding::solve(
            [tau](double t) {
