// bestfit ADDITION -- no upstream C# counterpart (sibling of models/model_spec.hpp).
//
...
namespace bestfit::analyses::support {
...
inline bestfit::estimation::SamplerType parse_runner_sampler(const std::string& s) {
    using ST = bestfit::estimation::SamplerType;
...
inline bestfit::numerics::distributions::ParameterEstimationMethod parse_estimation_method(
    const std::string& s) {
    using PEM = bestfit::numerics::distributions::ParameterEstimationMethod;
...
inline void apply_runner_bayes_knobs(bestfit::estimation::BayesianAnalysis& ba,
...
inline void apply_ordinates(bestfit::numerics::data::ProbabilityOrdinates& po,
...
inline void collect_uar(const bestfit::numerics::distributions::UncertaintyAnalysisResults& res,
...
inline ExtendedAnalysisResult run_composite(const models::spec::JsonValue& construct,
    ...
    for (const models::spec::JsonValue& f : model.at("families").items()) {
        auto dist = bestfit::numerics::distributions::create_distribution(f.as_string());
        auto child_model =
...
inline ExtendedAnalysisResult run_bootstrap(const models::spec::JsonValue& construct,
    ...
    auto dist = bestfit::numerics::distributions::create_distribution(model.at("family").as_string());
    auto method = parse_estimation_method(construct.value_or("estimation_method", "MaximumLikelihood"));
    auto* est = dynamic_cast<bestfit::numerics::distributions::IEstimation*>(dist.get());
    ...
    bestfit::numerics::BootstrapAnalysis boot(*dist, method, sample_size, replications, seed);
...
inline ExtendedAnalysisResult run_posterior_predictive(const models::spec::JsonValue& construct,
    ...
    bestfit::estimation::BayesianAnalysis ba(*udm);
