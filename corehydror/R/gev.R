...
dgev <- function(x, location = 0, scale = 1, shape = 0) {
  ch_gev_pdf_(as.double(x), location, scale, shape)
}

#' @rdname gev
#' @export
pgev <- function(q, location = 0, scale = 1, shape = 0) {
  ch_gev_cdf_(as.double(q), location, scale, shape)
}

#' @rdname gev
#' @export
qgev <- function(p, location = 0, scale = 1, shape = 0) {
  ch_gev_quantile_(as.double(p), location, scale, shape)
}
...
gev_moments <- function(location, scale, shape) {
  ch_gev_moments_(location, scale, shape)
}
...
gev_fit <- function(x, method = c("mle", "lmom", "mom")) {
  ...
  ch_gev_fit_(as.double(x), method)
}
...
gev_quantile_se <- function(p, location, scale, shape, sample_size) {
  sqrt(ch_gev_quantile_variance_(p, location, scale, shape, as.integer(sample_size)))
}
