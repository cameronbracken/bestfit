...
dist_pdf <- function(d, x) {
  check_dist(d)
  ch_dist_pdf_v_(d$family, d$params, as.double(x))
}
...
dist_log_pdf <- function(d, x) {
  check_dist(d)
  ch_dist_log_pdf_v_(d$family, d$params, as.double(x))
}
...
dist_cdf <- function(d, q) {
  check_dist(d)
  ch_dist_cdf_v_(d$family, d$params, as.double(q))
}
...
dist_quantile <- function(d, p) {
  check_dist(d)
  ch_dist_quantile_v_(d$family, d$params, as.double(p))
}
...
dist_random <- function(d, n, seed = NULL) {
  ...
  ch_dist_random_(d$family, d$params, as.integer(n), seed)
}
...
dist_moments <- function(d) {
  check_dist(d)
  ch_dist_moments_(d$family, d$params)
}
...
dist_log_likelihood <- function(d, data) {
  check_dist(d)
  ch_dist_log_likelihood_(d$family, d$params, as.double(data))
}
