...
if (!nzchar(path)) stop("direction-numbers file not found in corehydror inst/extdata")

test_that("sobol dimension-2 first 10 points match C# oracle", {
  mat <- ns$ch_sobol_generate_(2L, 10L, path)
  expected <- rbind(
    c(0.5000, 0.5000),
    c(0.7500, 0.2500),
    c(0.2500, 0.7500),
...
test_that("sobol dimension-1 first point is 0.5", {
  ...
  mat <- ns$ch_sobol_generate_(1L, 1L, "")
  expect_equal(mat[1, 1], 0.5, tolerance = 0)
})

test_that("sobol skip_to(1) matches first sequential point", {
  first_seq <- ns$ch_sobol_generate_(2L, 1L, path)[1, ]
  first_skip <- ns$ch_sobol_skip_to_(2L, 1L, path)
  expect_equal(first_skip, first_seq, tolerance = 0)
})

test_that("sobol skip_to(5) matches 5th sequential point", {
  fifth_seq <- ns$ch_sobol_generate_(2L, 5L, path)[5, ]
  fifth_skip <- ns$ch_sobol_skip_to_(2L, 5L, path)
