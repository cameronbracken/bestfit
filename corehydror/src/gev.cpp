...
// The core headers are vendored under src/corehydro_core/include (a symlink into core/; regenerate real files with tools/materialize_core.py).
#include <cpp11.hpp>
...
[[cpp11::register]]
double ch_gev_quantile_variance_(double p, double location, double scale, double shape,
                                 int sample_size) {
    dist::GeneralizedExtremeValue g(location, scale, shape);
...
[[cpp11::register]]
doubles ch_gev_parameter_covariance_(double location, double scale, double shape,
                                     int sample_size) {
    dist::GeneralizedExtremeValue g(location, scale, shape);
