...
// copula_factory.hpp's header comment -- so a single generic ch_cop_val_ (construct +
// evaluate) and ch_cop_fit_ (fit + return fitted params) cover every copula target with no
...
// class (not part of IBivariateCopula/IArchimedeanCopula), so ch_cop_fit_ dynamic_casts by
...
// marginX, marginY)` ctor path), mirroring ch_cop_fit_'s fitted-marginals convention: ""
// means no marginal for that side.
[[cpp11::register]]
double ch_cop_val_(std::string type, doubles params, std::string method, doubles args,
...
[[cpp11::register]]
list ch_cop_fit_(std::string type, doubles x, doubles y, std::string method, std::string marg_x,
                  std::string marg_y) {
